var StackPlot = React.createClass({
	...
	render: function() {
		...
		return React.createElement("g", {className: classes, transform: geom.transform.begin().translate(this.props.x, this.props.y).end(), onMouseOver: this.props.onMouseOver_})
	}
})
  	...
  	render: function () {
		...
		var matrixCells = that.state.meta.attrs.map(function(attrX, i) {
			...
			return that.state.meta.attrs.map(function(attrY, j) {
				var y = (h + dy) * j
				return React.createElement(StackPlot, React.__spread({data: that.state.data},  that.props.stackPlotConfiguration, {attrX: attrX, attrY: attrY, x: x, y: y, onMouseOver_: _.partial(that.handleMouseOverCell, i, j), focused: i== that.state.focus.r && j == that.state.focus.c}))
			})
		})
